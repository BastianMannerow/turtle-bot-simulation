...
class JohannesAgent:
    ...
    def __init__(self, environ):
        ...
        self.goal_phases = ["search", "move", "decide", "avoid", "walk_through"]
    ...
    def build_agent(self, agent_list):
        ...
        actr.chunktype("goal", "pos_x pos_y state")
        actr.chunktype("self", "pos_x pos_y")
        locations = {"Obstacle1": (0,0),... TODO}  # Define actual coordinates
        STATES = ["unknown", "obstacle_fake", "obstacle_not_fake", "goal_reached", "goal_not_reached"]
        actr_agent.set_goal(name="imaginal", delay=0)
        for location in locations:
            actr_agent.decmem.add(actr.chunkstring(string=f"isa obstacle pos_x {location[0]} y {location[1]} state {STATES[0]}"))
